import {Disposable, Disposer, Remover, NoopRemover, PMap, addListener, log} from "../core/util"
  ...
  wasAdded (host :Host) {
    ...
    const uninteract = host.interact.addProvider({
      ...
      handlePointerDown: (event, pos, into) => {
        ...
        for (const handler of this._gestureHandlers) {
          const iact = handler(event, pos)
